		...
		if (oConvertUtils.isNotEmpty(keyword)) {
			...
			if (oConvertUtils.isNotEmpty(keyword)) {
				...
				if (keyword.contains(SymbolConstant.COMMA)) {
					//update-begin--author:liaoxingwang--date:20220105--for：JTC-529【表单设计器】 编辑页面报错，in参数采用双引号导致 ----
					String inKeywords = "'" + String.join("','", keyword.split(",")) + "'";
					//update-end--author:liaoxingwang--date:20220105--for：JTC-529【表单设计器】 编辑页面报错，in参数采用双引号导致----
